...
namespace Movement
{
    public class Displacement
    {
        ...
        {
            // Check if the character was moving in one direction, and tried to go in the opposite direction. (Was the rigid bodies velocity positive while the movement vector was negative, or viceversa?)
            switchedDirections = WasMoving() && ((rb.velocity.z > 0 && movementVector.z < 0) || (rb.velocity.z < 0 && movementVector.z > 0));

            // If the character did switch directions, decelerate. Within the deceleration logic is the logic to start accelerating in the oposite direction once the velocity reached is 0.
            if (switchedDirections)
            {
                // I keep the current movement going in the same direction until they reach velocity 0 to simulate inertia. 
                // Once velocity 0 is reached, there's logic in the deceleration function that will turn the current movement around.
                currentMovement = new Vector3(0f, 0f, movementVector.z * -1);
                Decelerate();
            }
            // If they haven't switched directions, accelerate.
            else
            {
                currentMovement = new Vector3(0f, 0f, movementVector.z);
                Accelerate();
            }
        }
        ...
        {
            float velocityThreshold = 0.001f;
            return rb.velocity.z >= velocityThreshold || rb.velocity.z <= -velocityThreshold;
        }
        ...
        private IEnumerator LerpSpeedDown()
        {
            ...
            {
                speedLerpValue = 0;
                // If they switched directions, decelerate will be called. When the deceleration is over, we switch the movement direction and accelerate.
                // This ensures the character can't keep the same momentum when switching directions. They must decelerate to 0, switch, and then accelerate again.
                if (switchedDirections)
                {
                    currentMovement = new Vector3(currentMovement.x, currentMovement.y, currentMovement.z * -1);
                    Accelerate();
